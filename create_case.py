...
api_file = r"C:\Users\56835\Desktop\\api_1.txt"
auto_case = r"C:\Users\56835\Desktop\\api_2.txt"
...
def create_case():
    with open(api_file, mode='r+', encoding='utf-8') as data:
        for line in data:
            ...
            default_param = dict.copy(init_param)
            matchers = create_case_pattern.findall(str(default_param))
            for matcher in matchers:
                default_param = str(default_param).replace(str(matcher), str('\"'))
            case_data['request_param'] = default_param
            default_case_name = str(api_id) + "的默认入参"
            case_data['case_name'] = default_case_name
            # 写入自动化生成用例的文件中
            with open(auto_case, mode='a+', encoding='utf-8') as default:
                default.write(str(case_data) + "\n")
            ...
            # 规则11: 根据replace_param替换参数
            if "replace_param" in init_data:
                for param_dic in init_data["replace_param"]:
                    ppop=param_dic.keys()
                    key = list(param_dic.keys())[0]
                    temp_param = dict.copy(init_param)
                    for value in param_dic[key]:
                        temp_param[key] = value
                        case_data['request_param'] = temp_param
                        replace_key_case_name = str(api_id) + "中" + key + "替换为" + str(value)
                        case_data['case_name'] = replace_key_case_name
                        with open(auto_case, mode='a+', encoding='utf-8') as replace_key:
                            replace_key.write(str(case_data) + "\n")
